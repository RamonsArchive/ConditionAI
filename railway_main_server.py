...
class GoodDealsConditionAPI(LitAPI):
    ...
    def predict(self, request: ConditionRequest) -> ConditionResponse:
        ...
        try:
            ...
            # okay I need to predict the object first with ONLY THE TITLE HERE
             # Step 1: Detect object
            if hasattr(self, 'ollama_available') and self.ollama_available:
                object_data = self._detect_object_with_ollama_sync(request.search_query)
            else:
                object_data = self._detect_object_fallback(request.search_query)
            ...
            for item in request.results:
                try:
                    enhanced_item = self._process_item_sync(item, object_data)
    ...
    def _process_item_sync(self, item: ListingItem, object_data: Dict[str, Any]) -> EnhancedListingItem:
...
Example: "mountain bike in san diego" → bike
...
Output one word only: {search_query}"""
    ...
    def _assess_condition_sync(self, image_url: str, detected_object: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Extract only the condition level (excellent, good, fair, poor)
            condition_text = results[0][0]
            condition_level = condition_text.split(" in ")[1].split(" condition")[0]
            confidence = round(float(results[0][1]), 3)
            
            print(f"✅ CLIP assessment complete: {condition_level} (confidence: {confidence})")
...
if __name__ == "__main__":
    ...
    async def assess_conditions(request_data: ConditionRequest):
        try:
    ...
    async def process_csv():
        ...
        try:
            ...
            search_query = "mountain bike in san diego"
            ...
            condition_request = ConditionRequest(search_query=search_query, results=items)
